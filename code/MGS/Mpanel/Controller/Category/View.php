...
class View extends \Magento\Catalog\Controller\Category\View
{
    ...
    public function execute()
    {
                ...
                if($this->getRequest()->isAjax() && !$this->getRequest()->getParam('shopbyAjax')){
			...
			if ($category) {
				...
				$productlist = $this->_view->getLayout()->getBlock('category.products.list')->toHtml();
				$this->getResponse()->setBody($productlist);
			}
		}else{
			if ($this->_request->getParam(\Magento\Framework\App\ActionInterface::PARAM_NAME_URL_ENCODED)) {
				return $this->resultRedirectFactory->create()->setUrl($this->_redirect->getRedirectUrl());
