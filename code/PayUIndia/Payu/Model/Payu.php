...
class Payu extends \Magento\Payment\Model\Method\AbstractMethod {
    ...
    //validate response
    public function validateResponseOriginal($returnParams) {
		$order = $this->checkoutSession->getLastRealOrder();
		...
		if(isset($returnParams['hash'])){			
			...
			$saltString     = $this->getConfigData('salt').'|'.$status.'|'.$reverseKeyString;
                        ...
                        //if(isset($returnParams['additionalCharges'])) {
                        //      $hashString = $returnParams['additionalCharges'] . '|' . $hashString;
                        //}
    ...
    public function postProcessing(\Magento\Sales\Model\Order $order,
        ...
        //if($this->verifyPayment($order,$payment,$response['txnid']))
	//	{			
	...
	public function verifyPayment(\Magento\Sales\Model\Order $order,
            \Magento\Framework\DataObject $payment,$txnid)
	{
			...
			else 
			{
				...
				{	
					...
					{	
						...
						$order->addStatusToHistory($order->getStatus(), 'PayU Payment Verification Successful...');
						$order->save();						
					} */
					return true;
				}
				...
				elseif($res['status'] == 'pending' || $res['status'] == 'failure')
				{
					if($order->getState() == \Magento\Sales\Model\Order::STATE_PROCESSING)
					{
						...
						$order->setState(\Magento\Sales\Model\Order::STATE_PENDING, true);
						$order->setStatus(\Magento\Sales\Model\Order::STATE_PENDING);
						$order->addStatusToHistory($order->getStatus(), 'PayU Payment Verification Failed...');
						$order->save();					
					}
				}	*/
