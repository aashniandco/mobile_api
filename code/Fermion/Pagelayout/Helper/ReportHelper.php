...
class ReportHelper extends \Magento\Framework\App\Helper\AbstractHelper {            
    ...
    public function getResultForSalesresult($filter,$request_param) {
        try{
            $user = $this->authSession->getUser()->getEmail() ;
            ...
            $product_type = isset($request_param['product_type']) ? $request_param['product_type'] : "";
            ...
            if($filter != ''){
                foreach($filter as $key => $value)
                {
                      ...
                      if($filtervalue == 'size')
                    {
                      ...
                      $selectparam .= ",eaov20.value AS 'size'";
                      $attribute_id = $this->getAttributeIdByName('size');
                      $leftjoin .= ' LEFT JOIN catalog_product_entity_int cpei9 ON cpe.entity_id = cpei9.entity_id AND cpei9.`attribute_id` = '.$attribute_id.' LEFT JOIN eav_attribute_option_value eaov20 ON cpei9.value = eaov20.option_id';
                      $selectArr[] = 'size';
                ...
                if($filtervalue == 'bridal')
                {
                 ...
                 {
                     $leftjoin .= ' LEFT JOIN catalog_product_super_link cpsl on cpsl.product_id=cpe.entity_id' ;
                 $selectparam .= ",CASE WHEN (select distinct(value) from catalog_product_entity_varchar where attribute_id =87 and entity_id = cpsl.parent_id)!='no_selection' THEN CONCAT('https://aashniandco.com/media/catalog/product',(select distinct(value) from catalog_product_entity_varchar where attribute_id =87 and entity_id = cpsl.parent_id)) END AS 'image_link'" ;
                 $selectArr[] = 'image_link';
                 }

                    if($product_type=='configurable')
                 {
                     
                 $selectparam .= ",CASE WHEN (select distinct(value) from catalog_product_entity_varchar where attribute_id =87 and entity_id = cpe.entity_id)!='no_selection' THEN CONCAT('https://aashniandco.com/media/catalog/product',(select distinct(value) from catalog_product_entity_varchar where attribute_id =87 and entity_id = cpe.entity_id)) END AS 'image_link'" ;
                 $selectArr[] = 'image_link';
                 }
                ...
                if($newString != ''){
                    ...
                    $whereClause = " WHERE cpe.type_id = '$product_type' and eaov5.value in($newString)";
                }
                ...
                $this->conn->insert($tableName, [
                    ...
                    'user' => $user,
                    'designer' => str_replace(' ', '-',$designerNames),
