...
class FacebookSignIn implements \Fermion\NativeApp\Api\FacebookSigninInterface
{
	...
	public function signInWithFacebook($customerDetails)
	{
        ...
        $this->helper->validateRequests('/rest/V1/app_facebook_signin', $requestJson);
		...
		try{
        	...
        	if(!isset($customerDetails['email']) || (isset($customerDetails['email']) && empty($customerDetails['email']))) {
        		$customerDetails['email'] = $this->_getRandomEmail('facebook');
        	}
        	$this->helper->signUpCustomer($customerDetails, 'facebook');
		}
		catch(Exception $e){
